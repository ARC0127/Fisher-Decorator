...
def make_env_and_datasets(env_name, frame_stack=None, action_clip_eps=1e-5):
    ...
    if 'singletask' in env_name:
        # OGBench.
        ogbench_data_dir = os.environ.get('OGBENCH_DATA_DIR', None)
        if ogbench_data_dir is not None:
            env, train_dataset, val_dataset = ogbench.make_env_and_datasets(
                env_name, dataset_dir=ogbench_data_dir
            )
            eval_env = ogbench.make_env_and_datasets(
                env_name, env_only=True, dataset_dir=ogbench_data_dir
            )
        else:
